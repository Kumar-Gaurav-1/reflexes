...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Performance: Precalculate corner static indices to prevent loop garbage collection
      const p1 = (5 * 160 + 5) * 4;
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Performance: Clamp loop bounds outside to avoid conditional check per inner pixel
          ...
          // Performance: y loop outer to ensure contiguous memory access
