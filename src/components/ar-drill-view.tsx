...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // ⚡ Bolt Optimization: Unroll static array and .forEach to avoid GC micro-stutters
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // ⚡ Bolt Optimization: Pre-clamp boundaries outside loop, and traverse row-major (y then x) for better cache locality
