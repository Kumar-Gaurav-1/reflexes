...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Optimized: Unrolled loop avoids GC allocation of array/objects on every frame
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Optimized bounds for cache locality and inner loop checks
          ...
          // Optimized: Outer loop is Y to improve cache locality for row-major array
