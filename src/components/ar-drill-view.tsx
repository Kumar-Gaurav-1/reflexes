...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Optimized: Unrolled loop to prevent array allocation and forEach overhead in rAF
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Pre-clamp boundaries to avoid if checks in inner loop
          ...
          // Optimized: Swapped y and x loops for row-major cache locality
