...
// Pre-calculated static offsets for corner sampling to avoid dynamic array allocation in loop
// Formula: ((y * 160) + x) * 4
...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      for (let i = 0; i < CORNER_SAMPLE_OFFSETS.length; i++) {
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Pre-clamp boundary conditions to avoid branching within the nested loops
