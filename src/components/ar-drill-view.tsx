...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Unrolled static array iterations to avoid allocations and garbage collection
      ...
      if (globalMotionSum < 400) {
        ...
        for (let i = 0; i < activeTargets.length; i++) {
          ...
          // Pre-calculate and clamp loop boundaries outside the nested loop
          ...
          // Optimized for cache locality by swapping inner/outer loops (y then x)
