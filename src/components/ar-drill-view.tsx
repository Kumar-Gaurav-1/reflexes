...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Optimized: Unrolled corner loops to prevent object allocations and GC pauses in rAF loop
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Optimized: Clamped bounds outside the loop, structure loop row-major (y then x) for better cache locality
