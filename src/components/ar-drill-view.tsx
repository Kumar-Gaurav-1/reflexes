...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Optimization: Unroll array and loops to prevent GC stutters
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Pre-clamp boundaries outside the nested loops
          ...
          // Optimization: Iterate over y (rows) in outer loop for better cache locality
