...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Unrolled corner sampling to prevent GC allocation in requestAnimationFrame
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Optimized for row-major cache locality (y outer, x inner loop)
          // Pre-calculated boundaries to skip bounds checking per pixel
