      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        <div className="absolute inset-0 z-40 pointer-events-none">
          {status === 'active' && targets.map(t => (
            <div
              key={t.id}
              className={cn("absolute w-32 h-32 -ml-16 -mt-16 rounded-full glass-pro flex items-center justify-center transition-all", t.type === 'ball' ? 'animate-ball-throw' : 'animate-pulse-ring')}
              style={{ 
                left: `${t.x}%`, 
                top: `${t.y}%`,
                ...(t.type !== 'ball' && {
