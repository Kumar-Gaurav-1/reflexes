...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // ⚡ Bolt: Unrolled corner samples to prevent array allocation & .forEach GC stutters in rAF
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // ⚡ Bolt: Clamped bounds outside the loop and swapped to row-major iteration for cache locality
