...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // ⚡ Bolt: Unroll static array and .forEach to avoid GC micro-stutters in render loop
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // ⚡ Bolt: Clamp array boundaries before loop to avoid per-iteration bounds checks
          ...
          // ⚡ Bolt: Y outer, X inner for row-major cache locality in typed array access
