...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // ⚡ Bolt Optimization: Unrolled GC-friendly corner checks (avoids array allocation + forEach per frame)
      let globalMotionSum =
        Math.abs(data[3220] - prevData[3220]) +       // (5 * 160 + 5) * 4 = 3220
        Math.abs(data[3820] - prevData[3820]) +       // (5 * 160 + 155) * 4 = 3820
        Math.abs(data[73620] - prevData[73620]) +     // (115 * 160 + 5) * 4 = 73620
        Math.abs(data[74220] - prevData[74220]);      // (115 * 160 + 155) * 4 = 74220
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // ⚡ Bolt Optimization: Hoisted bounds checking and loop reversed for cache locality
