...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // Unrolled corner sampling to prevent GC micro-stutters from array allocation/forEach
      // Top-left: (5, 5) -> (5 * 160 + 5) * 4 = 3220
      globalMotionSum += Math.abs(data[3220] - prevData[3220])
      // Top-right: (155, 5) -> (5 * 160 + 155) * 4 = 3820
      globalMotionSum += Math.abs(data[3820] - prevData[3820])
      // Bottom-left: (5, 115) -> (115 * 160 + 5) * 4 = 73620
      globalMotionSum += Math.abs(data[73620] - prevData[73620])
      // Bottom-right: (155, 115) -> (115 * 160 + 155) * 4 = 74220
      globalMotionSum += Math.abs(data[74220] - prevData[74220])
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // Clamp bounds outside loops to avoid continuous conditional checks
          ...
          // Loop Y first, then X to ensure contiguous row-major memory access, improving cache locality
