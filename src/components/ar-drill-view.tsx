...
export function ARDrillView({ sport, drillName, onComplete }: ARDrillViewProps) {
  ...
  const detectMotion = useCallback(() => {
    ...
    if (prevFrameRef.current) {
      ...
      // OPTIMIZATION: Unroll corner sampling to avoid array allocation and .forEach
      // which causes GC micro-stutters in requestAnimationFrame loops.
      const p1 = (5 * 160 + 5) * 4
      globalMotionSum += Math.abs(data[p1] - prevData[p1])
      const p2 = (5 * 160 + 155) * 4
      globalMotionSum += Math.abs(data[p2] - prevData[p2])
      const p3 = (115 * 160 + 5) * 4
      globalMotionSum += Math.abs(data[p3] - prevData[p3])
      const p4 = (115 * 160 + 155) * 4
      globalMotionSum += Math.abs(data[p4] - prevData[p4])
      ...
      if (globalMotionSum < 400) {
        ...
        activeTargets.forEach(target => {
          ...
          // OPTIMIZATION: Pre-clamp loop bounds to eliminate conditional bounds checking
          // on every iteration.
          ...
          // OPTIMIZATION: Structure nested loops with Y (rows) as outer and X (cols)
          // as inner loop for cache-friendly contiguous memory access in row-major arrays.
